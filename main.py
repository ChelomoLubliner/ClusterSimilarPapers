...
if __name__ == '__main__':
    ...
    # Load the data
    df = load_data(file_path = 'arxiv-metadata-oai-snapshot.json')

    # Create a stemmer object
    stemmer = PorterStemmer()
    #df['abstract'] = df['abstract'].apply(lambda x: ' '.join([stemmer.stem(word) for word in x.split()]))
    ...
    df[['id','Topic']].to_csv('ids_topic.csv')
    # Tokenize topics
    ...
    import pickle
    with open('top_n_words.pkl', 'wb') as handle:
